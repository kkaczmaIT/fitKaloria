<?php
    ...
    class Dish
    {
        ...
        {
            $this->db->loadQuery('SELECT ID, name, amount_calories, mount_portion, recommend_portion FROM DISHES');
        ...
        {
            $this->db->loadQuery('SELECT D.ID, D.name, D.amount_calories, D.amount_portion, D.recommend_portion, M.name FROM DISHES D INNER JOIN DISHES_DAY DD ON D.ID=DD.ID_dishes INNER JOIN MEAL M ON DD.ID_meal=M.ID WHERE year(DD.date)=:_year AND month(DD.date)=:_month AND day(DD.date)=:_day AND DD.ID_user=:_ID_user AND DD.ID_meal=:_ID_meal');
        ...
        {
            $this->db->loadQuery('SELECT D.ID, D.name, D.amount_calories, D.amount_portion, D.recommend_portion, M.name FROM DISHES D INNER JOIN FITNESS_PLAN_DISHES_INGREDIENTS FPDI ON D.ID=FPDI.ID_dishes INNER JOIN  FITNESS_PLAN FP ON FDPI.ID_fitness_plan=FP.ID WHERE FPDI.ID_fitness_plan=:_ID_fitness_plan AND FPDI.ID_meal=:_ID_meal');
